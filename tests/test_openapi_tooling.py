...
def test_repository_coverage_maps_every_pinned_operation() -> None:
    ...
    international_did_operations = [
        operation for operation in report["operations"] if operation["path"].startswith("/didinter")
    ]
    ...
    fax_operations = [
        operation
        for operation in report["operations"]
        if operation["path"] == "/fax" or operation["path"].startswith("/fax/")
    ]
    assert len(fax_operations) == 12
    assert all(operation["response_model"] for operation in fax_operations)
    assert all(operation["unit_tested"] for operation in fax_operations)
    enterprise_fax_operations = [
        operation for operation in report["operations"] if operation["path"].startswith("/fax2")
    ]
    assert len(enterprise_fax_operations) == 21
    assert all(operation["response_model"] for operation in enterprise_fax_operations)
    assert all(operation["unit_tested"] for operation in enterprise_fax_operations)
    report_paths = ("/callhistory", "/faxhistory", "/faxdownload")
    report_operations = [
        operation for operation in report["operations"] if operation["path"] in report_paths
    ]
    ...
    server_operations = [
        operation for operation in report["operations"] if operation["path"].startswith("/server")
    ]
    assert len(server_operations) == 10
    assert all(operation["response_model"] for operation in server_operations)
    assert all(operation["unit_tested"] for operation in server_operations)
